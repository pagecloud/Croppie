...
(function(root, factory) {
  ...
  function _initializeResize() {
    ...
    function mouseDown(ev) {
      ...
      if (self.options.resizeControls.NSEW) {
        var matches = ev.currentTarget.className.match(
          /cr-resizer-handle-(\w+)/
        );
        direction = matches && matches[1];
        originalRatio =
          self.options.viewport.height / self.options.viewport.width;
      } else {
        direction =
    ...
    function isViewportValid(bounds) {
      var newBounds = ["height", "width", "top", "left"].reduce(function(
        dims,
        prop
      ) {
      ...
      {});
      return (
        newBounds.left >= minLeft &&
        newBounds.top >= Math.floor(minTop) &&
        newBounds.top + newBounds.height <=
          Math.ceil(minTop) + Math.ceil(maxHeight) &&
        newBounds.left + newBounds.width <=
          Math.ceil(minLeft) + Math.ceil(maxWidth) &&
    ...
    function applyBounds(nextBounds) {
      Object.keys(nextBounds).forEach(function(key) {
        self.options.boundary[key] +=
          nextBounds[key] - self.options.viewport[key];
  ...
  function _onZoom(ui) {
    var self = this,
      ...
      vpRect = ui
        ? ui.viewportRect
        : getInnerDimensions(self.elements.viewport),
  ...
  function _bind(options, cb) {
    ...
    return loadImage(url, hasExif, self.options.anonymousCrossOrigin).then(
      function(img) {
